...
def run_cutlass_moe_w4a16(
    ...
    | None = None,  # Optional for asymmetric W4A16 (e.g., AWQ/GPTQ)
    ...
    assert out_dtype in [torch.float16, torch.bfloat16], (
        f"Invalid output dtype: {out_dtype}"
    )
    if expert_map is not None:
        assert expert_num_tokens is None
    assert not use_batched_format, "batched format not supported yet"
    ...
    # In W4A16, without the intermediate quant_out step, we can place mm2_out in
    # workspace13 to naturally alternate workspaces and prevent overlap with act_out.
...
class CutlassExpertsW4A16(mk.FusedMoEExpertsModular):
    ...
    @staticmethod
    def _supports_parallel_config(
        moe_parallel_config: mk.FusedMoEParallelConfig,
    ) -> bool:
        raise NotImplementedError(
            "CutlassExpertsW4A16 is not yet used by an Oracle. "
            "This method should not be called."
...
def cutlass_moe_w4a16(
    ...
    - w1_q (torch.Tensor): The first set of 4-bit packed expert weights.
        Shape: [num_experts, 2*N, K // pack_factor]
        (pack_factor = 8 for int32 container)
