...
def run_cutlass_moe_w4a16(
    output: torch.Tensor,
    hidden_states: torch.Tensor,
    w1: torch.Tensor,
    w2: torch.Tensor,
    topk_ids: torch.Tensor,
    activation: "MoEActivation",  # Assuming defined in outer scope
    global_num_experts: int,
    expert_map: torch.Tensor | None,
    ...
    topk_weights: torch.Tensor | None,
    group_size: int,
    w1_zeros: torch.Tensor | None = None,  # Optional for asymmetric W4A16 (e.g., AWQ/GPTQ)
    ...
    assert w2.dtype == torch.int32
    assert w1.size(0) == w2.size(0), "Weights expert number mismatch"
    assert out_dtype in [torch.float16, torch.bfloat16], f"Invalid output dtype: {out_dtype}"
    if expert_map is not None:
        assert expert_num_tokens is None
    assert not use_batched_format, "batched format not supported yet"
    assert group_size == 32, f"Only group size 32 supported but got {group_size=}"
    ...
    # Standard workspace allocation (No float8_e4m3fn reinterpretations needed)
    a1_perm = _resize_cache(workspace2, (M * topk, K))
    mm1_out = _resize_cache(workspace13, (M * topk, N * 2))
    act_out = _resize_cache(workspace2, (M * topk, N))
    
    # In W4A16, without the intermediate quant_out step, we can place mm2_out in workspace13 
    # to naturally alternate workspaces and prevent overlap with act_out.
    mm2_out = _resize_cache(workspace13, (M * topk, K))
    ...
    # Permuted a1 reuses workspace2. No activation scales to pass.
    a1_perm, _, expert_first_token_offset, inv_perm, _ = moe_permute(
        a1,
        None,  # No a_scale for W4A16
        topk_ids,
        num_expert,
        local_E,
        expert_map,
    ...
    # MoE Activation (e.g., SwiGLU)
    apply_moe_activation(activation, act_out, mm1_out)

    # Second Expert GEMM (act_out directly consumed, no FP8 scaled quantization step)
    ops.cutlass_w4a16_moe_mm(
        mm2_out,
        act_out,
        w2,
    ...
    # Unpermute and aggregate the output
    moe_unpermute(
        out=output,
        permuted_hidden_states=mm2_out,
        topk_weights=topk_weights,
...
class CutlassExpertsW4A16(mk.FusedMoEExpertsModular):
    def __init__(
        self,
        out_dtype: torch.dtype | None,
        a_strides1: torch.Tensor,
    ...
    def _supports_current_device() -> bool:
        raise NotImplementedError(
            "CutlassExpertsW4A16 is not yet used by an Oracle. "
            "This method should not be called."
        )
    ...
    @staticmethod
    def _supports_quant_scheme(
        weight_key: mk.QuantKey | None,
        activation_key: mk.QuantKey | None,
    ) -> bool:
        raise NotImplementedError(
            "CutlassExpertsW4A16 is not yet used by an Oracle. "
            "This method should not be called."
    ...
    @staticmethod
    def _supports_activation(activation: mk.MoEActivation) -> bool:
        raise NotImplementedError(
            "CutlassExpertsW4A16 is not yet used by an Oracle. "
            "This method should not be called."
        )
    ...
    @staticmethod
    def _supports_parallel_config(moe_parallel_config: mk.FusedMoEParallelConfig) -> bool:
        raise NotImplementedError(
            "CutlassExpertsW4A16 is not yet used by an Oracle. "
            "This method should not be called."
        )
    ...
    def apply(
        ...
        # W4A16 handles activations natively in 16-bit, so act scales should be None
        assert a1q_scale is None, "a1q_scale is not used in W4A16 MoE"
        assert a2_scale is None, "a2_scale is not used in W4A16 MoE"
        ...
        # Note: the underlying backend binding `run_cutlass_moe_w4a16` strips out 
        # the FP8 specific alphas/act_scales and only relies on the weight group scales.
        run_cutlass_moe_w4a16(
            output,
            hidden_states,
            w1,
            w2,
...
def cutlass_moe_w4a16(
    ...
    quant_config: mk.FusedMoEQuantConfig,
    moe_config: mk.FusedMoEConfig,
    activation: mk.MoEActivation = mk.MoEActivation.SILU,
    expert_map: torch.Tensor | None = None,
    apply_router_weight_on_input: bool = False,
    global_num_experts: int = -1,
    group_size: int = 32,
    ...
    """
    This function computes a w4a16-quantized Mixture of Experts (MoE) layer
    using two sets of 4-bit packed weights, w1_q and w2_q, 16-bit activations,
    and a top-k gating mechanism. The matrix multiplications are implemented 
    with CUTLASS mixed-dtype grouped gemm.

    Parameters:
    - a (torch.Tensor): The input tensor to the MoE layer.
        Shape: [M, K], dtype: bfloat16 or float16
    - w1_q (torch.Tensor): The first set of 4-bit packed expert weights.
        Shape: [num_experts, 2*N, K // pack_factor] (pack_factor = 8 for int32 container)
    - w2_q (torch.Tensor): The second set of 4-bit packed expert weights.
        Shape: [num_experts, K, N // pack_factor]
    - topk_weights (torch.Tensor): The weights of each token->expert mapping.
    - topk_ids (torch.Tensor): The token->expert mappings.
    - a_strides1 (torch.Tensor): The input strides for the first gemm.
        Shape: [num_experts]
    - a_strides2 (torch.Tensor): The input strides for the second gemm.
        Shape: [num_experts]
    - b_strides1 (torch.Tensor): The packed layout for the first gemm weights.
        Shape: [num_experts, 3], dtype: torch.int32
    - b_strides2 (torch.Tensor): The packed layout for the second gemm weights.
        Shape: [num_experts, 3], dtype: torch.int32
    - c_strides1 (torch.Tensor): The output strides for the first gemm.
        Shape: [num_experts]
    - c_strides2 (torch.Tensor): The output strides for the second gemm.
        Shape: [num_experts]
    - s_strides1 (torch.Tensor): strides for the group-wise scales for the first gemm.
        Shape: [num_experts, 2], dtype: torch.int64
    - s_strides2 (torch.Tensor): strides for the group-wise scales for the second gemm.
        Shape: [num_experts, 2], dtype: torch.int64
    - activation (MoEActivation): The activation function to use.
    - expert_map (Optional[torch.Tensor]): In the case of Expert parallel,
    ...
    - apply_router_weight_on_input (bool): When true, the topk weights are
        applied directly on the inputs. This is only applicable when topk is 1.
    - global_num_experts (int): The total number of experts.
    - group_size (int): The number of weights per scale factor (e.g., 32).
