...
class CompressedTensorsW4A16CutlassMoEMethod(CompressedTensorsMoEMethod):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        # For W4A16, these remain in their original 16-bit dtype (bf16/fp16).
        w13_weight_scale = torch.nn.Parameter(
            torch.ones(
                num_experts,
                2 * intermediate_size_per_partition,
    ...
    def process_weights_after_loading(self, layer):
        ...
        w13_weight_scale_packed = layer.w13_weight_scale.permute(0, 2, 1).contiguous()
        replace_parameter(layer, "w13_weight_scale", w13_weight_scale_packed)
        w2_weight_scale_packed = layer.w2_weight_scale.permute(0, 2, 1).contiguous()
    ...
    def select_gemm_impl(
        ...
        from vllm.model_executor.layers.fused_moe import CutlassExpertsW4A16

        experts: FusedMoEExpertsModular

        logger.debug("CutlassExpertsW4A16(%s)", self.__class__.__name__)
        experts = CutlassExpertsW4A16(
