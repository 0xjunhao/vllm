...
using MmaType = cutlass::bfloat16_t; // 16-bit Activations
using QuantType = cutlass::int4b_t;

// For BF16, 128 * 8 / 16 = 64
...
// Use 16-bit group scales for A16
...
using ArchTag = cutlass::arch::Sm90;  // Tag indicating the minimum SM
...
struct W4A16GroupedGemmKernel {
  ...
  // Standard EVT Epilogue (No per-channel or per-token scales for A16)
  ...
  static void grouped_mm(torch::stable::Tensor& out_tensors,
    ...
    run_get_group_gemm_starts(expert_offsets, a_ptrs, b_ptrs, out_ptrs,
                              b_group_scales_ptrs, a_tensors, b_tensors, 
                              out_tensors, b_group_scales, b_group_size);
    ...
    EpilogueArguments epilogue_arguments{
        {}, // Standard linear combination thread arguments
        nullptr,                                       // C
...
void mm_dispatch(torch::stable::Tensor& out_tensors,
  ...
  if (schedule == "Kernel_128x16_1x1x1_Coop") {
    Kernel_128x16_1x1x1_Coop::grouped_mm(
        ...
        expert_offsets, problem_sizes, a_strides, b_strides, c_strides, group_scale_strides);
...
STABLE_TORCH_LIBRARY_IMPL(_C, CUDA, m) {
  m.impl("cutlass_w4a16_moe_mm", TORCH_BOX(&mm));
}
